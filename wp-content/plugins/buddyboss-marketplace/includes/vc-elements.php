...
//                    ),
         ...
         public function featured_sellers($atts, $content = null) {
            ...
            if ( empty( $ids ) ) {
                ...
                foreach( $shops_favorited_count as $user_id => $s_favorite_count ) {
                    ...
                    if ( 'Vendor' == wcv_get_user_role( $user_id ) ) $highgest_stores[$user_id] = $s_favorite_count;
                    if ( $i > 1 ) break;
                    $i++;
                }
            } else {
                ...
                foreach ( $ids as $user_id ) {
                    ...
                    $highgest_stores[$user_id] = $shops_favorited_count[$user_id] ? $shops_favorited_count[$user_id] : 0;
                    if ( $i > 1 ) break;
                    $i++;
                }
            }
        ...
        public function product_feed($atts, $content = null) {
           ...
           /** store item data ************************************************************/
                ...
                if( is_array( $shops_favorited_count ) ) {
                  ...
                  arsort( $shops_favorited_count );
                   ...
                   foreach ( $shops_favorited_count as $user_id => $s_favorite_count ) {
                         ...
                         if ( 'Vendor' == wcv_get_user_role( $user_id ) ) {
                            $shop_author = $user_id;
                            break;
                         }
                     }
            ...
            $shop_favs_count = $shops_favorited_count[$user_id];
...
//                $html .= do_shortcode('[products ids="'.$ids.'" orderby="include"]');
        ...
        public function collections( $atts, $content = null ) {
            ...
            $cache_key  = ( function_exists( 'onesocial_cache_key' ) )
                ? onesocial_cache_key( 'collections', $atts, $timestamps ) : '';
            ...
            if ( false === ( $output = get_transient( $cache_key ) ) ) {
                ...
                if ( $total ) {
                    ...
                    foreach ( $featured as $index => $cat ) {
                        ...
                        ?>
                        <a href="<?php echo get_term_link( $cat->slug,
                            'product_cat' ); ?>"
                           class="bm-f-category block-<?php echo $image_sizes[ $i ][0]; ?>">
                            ...
                            if ( $thumbnail_id ) {
                                ...
                                $timestamps = array( '_product_updated' );
                                $cache_key
                                            = ( function_exists( 'onesocial_cache_key' ) )
                                    ? onesocial_cache_key( 'collections',
                                        array( 'terms' => $category_id ), $timestamps )
                                    : '';
                                if ( false === ( $products
                                    ...
                                    if ( ! empty( $cache_key ) ) {
                                        set_transient( $cache_key, $products,
                                            WEEK_IN_SECONDS );
                                    }

                                }

                                if ( isset( $products->post ) && has_post_thumbnail( $products->post->ID ) ) {
                                    echo get_the_post_thumbnail( $products->post->ID,
                                        $image_sizes[ $i ][1] );
                                } else {
