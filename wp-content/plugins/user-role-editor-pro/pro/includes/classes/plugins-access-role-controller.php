...
class URE_Plugins_Access_Role_Controller {
    ...
    public function __construct() {
        ...
        $this->access_data = get_option(URE_Plugins_Access_Role::ACCESS_DATA_KEY);
    ...
    public function load_data($role_id) {
        ...
        if (is_array($this->access_data) && array_key_exists($role_id, $this->access_data)) {
            $result =  $this->access_data[$role_id];
        } else {
            $result = array(
                'selection_model'=>1,
                'data'=>array(
    ...
    private function save_data($role_id) {
        ...
        if (!is_array($this->access_data)) {
            $this->access_data = array();
        }

        if (isset($wp_roles->roles[$role_id])) {
            $this->access_data[$role_id] = $access_for_role;
        } elseif (isset($this->access_data[$role_id])) {
            unset($this->access_data[$role_id]);
        }
        
        update_option(URE_Plugins_Access_Role::ACCESS_DATA_KEY, $this->access_data);
